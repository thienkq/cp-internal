...
export default async function BonusLeavePage() {
  ...
  const totalGranted = bonusLeaves.reduce(
    (sum, item) => sum + item.total_granted,
    0
  );
  const totalUsed = bonusLeaves.reduce((sum, item) => sum + item.total_used, 0);
  const totalRemaining = bonusLeaves.reduce(
      ...
      <div className='space-y-6'>
        ...
        <div>
          ...
          <p className='text-gray-600'>
            Grant and manage bonus leave days for employees
          </p>
        </div>
        ...
        <Card className='bg-blue-50 border-blue-200'>
          ...
          <CardContent className='text-blue-700'>
            <div className='space-y-2 text-sm'>
              <p>
                • <strong>Multiple Grants:</strong> You can grant multiple bonus
                leave periods to the same employee in the same year
              </p>
              <p>
                • <strong>Carryover:</strong> Bonus leave follows the same
                carryover rules as annual leave
              </p>
              <p>
                • <strong>Priority:</strong> Bonus leave is used before annual
                leave when employees request time off
              </p>
              <p>
                • <strong>Audit Trail:</strong> Each grant is tracked with
                reason and admin who granted it
              </p>
              <p>
                • <strong>Flexibility:</strong> You can delete individual grants
                if needed
              </p>
            </div>
          </CardContent>
